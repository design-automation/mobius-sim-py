...
class TestAttribs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        m.add_attrib(ENT_TYPE.POINT, 'some_str', DATA_TYPE.STR)
        m.set_attrib_val(point0, 'some_str', 'hello1')
        m.add_attrib(ENT_TYPE.POINT, 'some_number', DATA_TYPE.NUM)
        m.set_attrib_val(point1, 'some_number', 55)
        self.model = m

    def test_query_string(self):
        result = self.model.query(ENT_TYPE.POINT, 'some_str', '==', 'hello1')
    ...
    def test_query_number(self):
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '==', 55)
        self.assertEqual(list(result), ['pt1'])
        
    def test_query_xyz(self):
        result = self.model.query(ENT_TYPE.POSI, 'xyz', '==', [1,2,3])
    ...
    def test_query_lt_gt(self):
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '>', 50)
        self.assertEqual(list(result), ['pt1'])
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '>=', 55)
        self.assertEqual(list(result), ['pt1'])
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '<', 50)
        ...
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '<=', 50)
        self.assertEqual(list(result), [])
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '<=', 60)
        self.assertEqual(list(result), ['pt1'])

        
    def test_query_none(self):
        result = self.model.query(ENT_TYPE.POSI, 'xyz', '==', None)
        self.assertEqual(list(result), [])
        result = self.model.query(ENT_TYPE.POINT, 'some_str', '!=', None)
        self.assertEqual(list(result), ['pt0'])
        result = self.model.query(ENT_TYPE.POINT, 'some_str', '==', None)
        result.sort()
        self.assertEqual(list(result), ['pt1', 'pt2'])
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '!=', None)
        self.assertEqual(list(result), ['pt1'])
        result = self.model.query(ENT_TYPE.POINT, 'some_number', '==', None)
        result.sort()
